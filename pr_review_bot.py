...
# фоном на каждый PR, и большой потолок зря бронирует бюджет Феникса (~500k ток/мин),
# а когда-то провоцировал и таймауты.
...
def _fenix_request_with_retry(endpoint: str, payload: dict):
    ...
    Ретраим:
      - 429 (лимит ~500k ток/мин) — ждём Retry-After (или экспоненциальный backoff)
