...
Отдельно про `FENIX_MAX_TOKENS`. В `.env.example` стоит `4096` — это дефолт
вебхучного пути, где ревью идёт фоном на каждый PR и большой потолок зря
бронирует бюджет Феникса. Для консольного запуска ставится `16384`: прогон
ручной, файлов за раз больше, срочности нет, и обрезанный ответ обходится
дороже, чем забронированные токены. Признак, что потолок мал, — строка
`finish_reason=length` в логе: JSON тогда приходит битым и файл остаётся
без ревью.

`POST_MIN_SEVERITY` для консольного прогона обычно ставят в `suggestion`: ручной
запуск делают, когда хотят посмотреть всё, а не только критичное. Метка `@jarvis`
из описания PR при этом работает так же, как на вебхуке — код общий.
