"""
Детерминированный слой стайлгайда команды — метка `[styleguide]` (ARCHITECTURE.md §2.2).
...
Зачем отдельный модуль, если есть styleguide.md:
  ...
  • styleguide_rules.py — ДВИЖОК: гоняет regex-правила команды по добавленным строкам
    diff'а сам, мимо ИИ (0 токенов) → находки выходят под [styleguide].
...
def scan(diff_text: Optional[str], rules: list[Rule]) -> list[dict]:
    ...
    (маркер `[styleguide]` навешивает оркестратор; сниппет не включаем — стабильность
